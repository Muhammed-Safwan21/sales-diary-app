...
const mockPurchaseReturns = [
  ...
  },
  {
    ...
    items: 1,
    originalOrderNumber: 'PO-0004',
  },
  {
    id: '5',
    returnNumber: 'PR-0005',
...
export default function PurchaseReturnsScreen() {
  ...
  const handleViewPurchaseReturn = (returnId: string) => {
    // Navigate to view screen
    router.push(`/purchase/return/${returnId}`);
  };
      ...
      <SafeAreaView>
        <View style={styles.header}>
          ...
          <View style={styles.headerTitleContainer}>
            ...
            <Text style={styles.headerTitle}>Purchase Returns</Text>
          </View>
        ...
        <View style={styles.summaryContainer}>
          <View style={styles.summaryRow}>
            ...
            >
              ...
              <Text style={styles.summaryValue}>{purchaseReturns.length}</Text>
            </BlurView>
            ...
            >
              ...
              <Text style={styles.summaryValue}>
                ₹{purchaseReturns.reduce((sum, returnItem) => sum + returnItem.amount, 0).toLocaleString('en-IN')}
              </Text>
            </BlurView>
          </View>
        </View>
  ...
  const renderFilterTabs = () => (
    ...
    >
      ...
      >
        {[
          { key: 'all', label: 'All', count: purchaseReturns.length },
          {
            key: 'draft',
            label: 'Draft',
            count: purchaseReturns.filter((r) => r.status === 'draft').length,
          },
          {
            key: 'sent',
            label: 'Sent',
            count: purchaseReturns.filter((r) => r.status === 'sent').length,
          },
          {
            key: 'accepted',
            label: 'Accepted',
            count: purchaseReturns.filter((r) => r.status === 'accepted').length,
          },
          {
            key: 'completed',
            label: 'Completed',
            count: purchaseReturns.filter((r) => r.status === 'completed').length,
          },
          {
            key: 'cancelled',
            label: 'Cancelled',
            count: purchaseReturns.filter((r) => r.status === 'cancelled').length,
          },
        ].map((filter) => (
          <TouchableOpacity
            key={filter.key}
  ...
  const renderPurchaseReturnCard = (returnItem: any, index: number) => (
    <Animated.View
      key={returnItem.id}
    ...
    >
      <TouchableOpacity
        onPress={() => handleViewPurchaseReturn(returnItem.id)}
        activeOpacity={0.95}
      ...
      >
        ...
        >
          <View style={styles.cardContent}>
            ...
            {/* Items count info and original order */}
            <View style={styles.itemsInfo}>
              ...
              <Text style={[styles.originalOrderText, { color: theme.colors.textSecondary }]}>
                From: {returnItem.originalOrderNumber}
              </Text>
    ...
    <Animated.View entering={FadeIn.delay(300)} style={styles.emptyContainer}>
      ...
      >
        ...
        <Text style={[styles.emptyTitle, { color: theme.colors.text }]}>
          No {selectedFilter === 'all' ? '' : selectedFilter} purchase returns
        </Text>
        <Text
        ...
        >
          {selectedFilter === 'all'
            ...
            : `No ${selectedFilter} purchase returns found`}
        </Text>
      </BlurView>
    </Animated.View>
  );
  ...
  const renderFloatingActionButton = () => (
    ...
    >
      <TouchableOpacity
        ...
        onPress={() => router.push('/purchase/return/add')}
        activeOpacity={0.8}
      >
        ...
        >
          {isLoading ? (
            ...
            <View style={styles.returnsList}>
              {filteredPurchaseReturns.map((returnItem, index) =>
                renderPurchaseReturnCard(returnItem, index)
              )}
            </View>
          ) : (
            renderEmptyState()
