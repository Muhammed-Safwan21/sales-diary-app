...
export default function CreateInvoiceScreen() {
  ...
  const [autoProcessScan, setAutoProcessScan] = useState(true);
  const [isCreatingInvoice, setIsCreatingInvoice] = useState(false);
  const [paymentMethod, setPaymentMethod] = useState<'CASH' | 'CREDIT' | 'UPI' | 'BANK_TRANSFER'>('CASH');
...
const calculateItemTotals = (item: any) => {
  ...
  const itemDiscountPercentage = parseFloat(item.discountPercentage) || 0;
  const discountAmount = (subtotal * itemDiscountPercentage) / 100;
  const afterDiscount = subtotal - discountAmount;
  
  const itemTaxPercentage = parseFloat(item.taxPercentage) || 0;
  const taxAmount = (afterDiscount * itemTaxPercentage) / 100;
...
const calculateInvoiceTotals = () => {
  ...
  invoiceItems.forEach(item => {
    ...
    totalDiscountAmount += itemTotals.discountAmount;
    totalTaxAmount += itemTotals.taxAmount;
  });
  
  // Apply invoice-level discount
  const invoiceDiscountAmount = (subtotal * discountPercentage) / 100;
  const afterInvoiceDiscount = subtotal - invoiceDiscountAmount;
  
  // Apply invoice-level tax
  const invoiceTaxAmount = (afterInvoiceDiscount * taxPercentage) / 100;
  
  const finalAmount = afterInvoiceDiscount + invoiceTaxAmount;
  const roundOffAmount = Math.round(finalAmount) - finalAmount;
  const totalAmount = finalAmount + roundOffAmount;
  
  return {
    subtotal,
    totalDiscountAmount: totalDiscountAmount + invoiceDiscountAmount,
    totalTaxAmount: totalTaxAmount + invoiceTaxAmount,
    roundOffAmount,
    totalAmount
  };
};
...
const validateInvoiceData = () => {
  if (!invoiceDetails.customerName || !invoiceDetails.customerId) {
    Alert.alert('Validation Error', 'Please select a customer');
    return false;
  }
  
  if (invoiceItems.length === 0 || invoiceItems.every(item => !item.name)) {
    Alert.alert('Validation Error', 'Please add at least one item');
    return false;
  }
  
  if (invoiceItems.some(item => item.name && (!item.quantity || !item.price))) {
    Alert.alert('Validation Error', 'Please fill in quantity and price for all items');
    return false;
  }
...
const prepareInvoiceData = (isDraft: boolean = false): any => {
  ...
  const items: any[] = invoiceItems
    ...
    .map(item => {
      ...
      return {
        ...
        subtotal: Number(itemTotals.subtotal),
        discountPercentage: parseFloat(item.discountPercentage) || 0,
        discountAmount: Number(itemTotals.discountAmount),
        taxPercentage: parseFloat(item.taxPercentage) || 0,
        taxAmount: Number(itemTotals.taxAmount),
        totalAmount: Number(itemTotals.totalAmount),
  ...
  return {
    ...
    paymentMethod,
    isDraft,
    partyId: parseInt(invoiceDetails.customerId),
    financialYearId: 1, // You might need to get this from context/store
    branchId: 2, // You might need to get this from context/store
    staffId: 2, // You might need to get this from context/store
    adminId: 2, // You might need to get this from context/store
    subtotal: Number(totals.subtotal),
    discountPercentage:Number(discountPercentage),
    discountAmount: Number(totals.totalDiscountAmount),
    taxPercentage,
    taxAmount: Number(totals.totalTaxAmount),
    roundOffAmount: Number(totals.roundOffAmount),
    ...
    payments: {
      paymentMethod,
      ledgerId: 1, // You might need to get this from context/store
      amount: Number(totals.totalAmount),
      referenceNo: `INV-${invoiceDetails.invoiceNumber}`,
      notes: `Payment for invoice ${invoiceDetails.invoiceNumber}`
    }
  };
};
    ...
    >
      ...
      <View style={styles.formGroup}>
        ...
        <View style={styles.paymentMethodContainer}>
          {['CASH', 'CREDIT', 'UPI', 'BANK_TRANSFER'].map((method) => (
            <TouchableOpacity
              key={method}
              style={[
          ...
          >
            <View style={styles.shareButtonContent}>
              ...
              <Text style={styles.shareButtonText}>
                {isCreatingInvoice ? 'Creating...' : 'Save & Share'}
              </Text>
            </View>
          </LinearGradient>
        </TouchableOpacity>
  ...
  return (
    ...
    >
      ...
      <PartiesSelectionModal
        ...
        onSelectParty={handleCustomerSelect}
        selectedPartyId={invoiceDetails.customerId}
...
//       return await apiClient.post(API.INVOICES || '/v1/invoices', invoiceData, {
//         headers: {
//           Authorization: `Bearer ${process.env.EXPO_PUBLIC_API_TOKEN}`, // Replace with your auth token mechanism
//         },
//       });
//     },
//     onSuccess: (response) => {
...
//       isDraft: false,
//       partyId: parseInt(invoiceDetails.customerId),
//       financialYearId: 1, // Adjust based on app context
//       branchId: 2, // Adjust based on app context
//       staffId: 2, // Adjust based on app context
//       adminId: 2, // Adjust based on app context
//       subtotal: calculateTotal(),
//       discountPercentage: 1,
//       discountAmount: 1,
//       taxPercentage: 10,
//       taxAmount: calculateTotal() * 0.1,
//       roundOffAmount: 0.1,
//       totalAmount: calculateTotal() + calculateTotal() * 0.1 + 0.1,
//       notes: 'Cash sale invoice',
//       startDate: invoiceDetails.invoiceDate,
//       endDate: invoiceDetails.dueDate,
...
//         discountAmount: 1,
//         taxPercentage: parseFloat(item.taxPercentage || '0'),
//         taxAmount: parseFloat(item.amount) * 0.1,
//         totalAmount: parseFloat(item.amount) + parseFloat(item.amount) * 0.1,
//         notes: 'Item note',
//       })),
//       payments: {
//         paymentMethod: 'CASH',
//         ledgerId: 1, // Adjust based on app context
//         amount: calculateTotal() + calculateTotal() * 0.1 + 0.1,
//         referenceNo: `CS-${invoiceDetails.invoiceNumber}`,
//         notes: 'Paid in full by cash',
//       },
//     };
...
//                     style={[styles.totalValue, { color: theme.colors.text }]}
//                   >
//                     ₹{(calculateTotal() * 0.1).toLocaleString('en-IN', {
//                       minimumFractionDigits: 2,
//                       maximumFractionDigits: 2,
//                     })}
//                   </Text>
...
//                       ]}
//                     >
//                       {(calculateTotal() + calculateTotal() * 0.1 + 0.1).toLocaleString('en-IN', {
//                         minimumFractionDigits: 2,
//                         maximumFractionDigits: 2,
//                       })}
//                     </Text>
...
//                       ? 'rgba(255, 255, 255, 0.12)'
//                       : 'rgba(0, 0, 0, 0.08)',
//                   opacity: createInvoiceMutation.isPending ? 0.7 : 1,
//                 },
//               ]}
