...
const mockRefunds = [
  {
    ...
    originalReceipt: 'REC-001',
    reason: 'Damaged goods received. Customer returned items due to shipping damage.',
    items: [
      {
        id: 'i1',
    ...
    notes: 'Refund processed via bank transfer. Customer was satisfied with the resolution.',
    terms: 'Refund policy: 30 days from purchase date. Damaged goods are eligible for full refund.',
  },
];
...
export default function RefundView() {
  ...
  const refund = mockRefunds.find((r) => r.id === id) || mockRefunds[0];

  const totalAmount = refund.items.reduce((sum, item) => sum + item.refundAmount, 0);

  const handleEdit = () => {
    router.push(`/receipts/refund/form?id=${refund.id}`);
  };
  const handleDelete = () => {
    Alert.alert(
      'Delete Refund',
  ...
  const handleSend = () => {
    Alert.alert('Send Refund', 'Send refund confirmation to customer functionality coming soon.');
  };
        ...
        <SafeAreaView>
          <View style={styles.header}>
            ...
            <View style={styles.headerTitleContainer}>
              ...
              <Text style={[styles.headerTitle, { color: theme.colors.text }]}>Refund</Text>
            </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <BlurView intensity={themeType === 'dark' ? 15 : 80} tint={themeType} style={styles.section}>
          ...
          <View style={styles.detailRow}>
            ...
            <Text style={[styles.detailValue, { color: theme.colors.text }]}>{refund.customerName}</Text>
          </View>
          <View style={styles.detailRow}>
            <Text style={[styles.detailLabel, { color: theme.colors.text }]}>Original Receipt</Text>
            <Text style={[styles.detailValue, { color: theme.colors.text }]}>{refund.originalReceipt}</Text>
          </View>
          <View style={styles.detailRow}>
