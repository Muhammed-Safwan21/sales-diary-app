...
const mockEstimates = [
  ...
  {
    id: '3',
    estimateNumber: 'DRAFT-003',
    customerName: 'Bob Johnson',
    customerId: '3',
    amount: 3200.0,
    status: 'draft',
    estimateDate: '2025-01-22',
...
export default function EstimateListingScreen() {
  ...
  const handleDeleteEstimate = (estimateId: string, estimateNumber: string) => {
    Alert.alert(
      'Delete Estimate',
      `Are you sure you want to delete ${estimateNumber}? This action cannot be undone.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            setEstimates(
              estimates.filter((estimate) => estimate.id !== estimateId)
            );
          },
        },
      ]
  ...
  const handleEditEstimate = (estimateId: string) => {
    ...
    // router.push(`/estimate/form/${estimateId}`);
  };
  ...
  const filteredEstimates = estimates.filter((estimate) => {
    ...
    return estimate.status === selectedFilter;
  });
      ...
      >
        {[
          ...
          },
          {
            key: 'expired',
            label: 'Expired',
            count: estimates.filter((e) => e.status === 'expired').length,
          },
          {
            key: 'draft',
            label: 'Draft',
  ...
  const renderEstimateCard = (estimate: any, index: number) => (
    <Animated.View
      key={estimate.id}
    ...
    >
      ...
      >
        ...
        <TouchableOpacity
          ...
          onPress={() => handleViewEstimate(estimate.id)}
          activeOpacity={0.8}
        >
          <View style={styles.cardHeader}>
            <View style={styles.estimateNumberContainer}>
              <Text
                style={[styles.estimateNumber, { color: theme.colors.text }]}
              >
            ...
            <View style={styles.cardActions}>
              <TouchableOpacity
                ...
                onPress={() => handleEditEstimate(estimate.id)}
              >
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  handleDeleteEstimate(estimate.id, estimate.estimateNumber)
                }
              >
          ...
          <View style={styles.cardDetails}>
            <View style={styles.detailRow}>
              ...
              <View style={styles.detailItem}>
                ...
                >
                  {estimate.items} {estimate.items === 1 ? 'item' : 'items'}
                </Text>
              </View>
            </View>
          ...
          {estimate.status === 'expired' && (
            <View style={styles.expiredWarning}>
              ...
              <Text style={styles.expiredText}>
                ...
                {Math.ceil(
                  (new Date().getTime() -
                    new Date(estimate.validUntil).getTime()) /
                    (1000 * 60 * 60 * 24)
                )}{' '}
                days ago
              </Text>
    ...
    <Animated.View entering={FadeIn.delay(300)} style={styles.emptyContainer}>
      ...
      >
        ...
        <Text style={[styles.emptyTitle, { color: theme.colors.text }]}>
          No {selectedFilter === 'all' ? '' : selectedFilter} estimates found
        </Text>
        <Text
        ...
        >
          {selectedFilter === 'all'
            ...
            : `No ${selectedFilter} estimates at the moment`}
        </Text>
      </BlurView>
    </Animated.View>
  );
  ...
  const renderFloatingActionButton = () => (
    ...
    >
      <TouchableOpacity
        ...
        onPress={() => router.push('/estimate/form/create')}
        activeOpacity={0.8}
      >
          ...
          {isLoading ? (
            ...
            <View style={styles.estimatesList}>
              {filteredEstimates.map((estimate, index) =>
                renderEstimateCard(estimate, index)
              )}
            </View>
          ) : (
            renderEmptyState()
