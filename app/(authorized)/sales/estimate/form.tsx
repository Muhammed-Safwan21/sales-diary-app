...
export default function SalesEstimateForm() {
  const { theme, themeType }: any = useTheme();
  const router = useRouter();
  const [showDatePicker, setShowDatePicker] = useState<
    'estimateDate' | 'validTill' | null
  >(null);
  const [showCustomerDropdown, setShowCustomerDropdown] = useState(false);
  const [isCreatingEstimate, setIsCreatingEstimate] = useState(false);
  const [estimate, setEstimate] = useState<SalesEstimate>({
    customerId: '',
    customerName: '',
    estimateNumber: '',
  ...
  const calculateTotals = () => {
    ...
    const totalTax = estimate.items.reduce((sum, item) => sum + item.taxAmount, 0);
    const totalDiscount = estimate.items.reduce((sum, item) => sum + item.discountAmount, 0);
    const totalAmount = estimate.items.reduce((sum, item) => sum + item.total, 0);
  ...
  const removeItem = (id: string) => {
    if (estimate.items.length > 1) {
      setEstimate({
        ...estimate,
        items: estimate.items.filter((item) => item.id !== id),
      });
    }
  };
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
      console.log('Estimate submitted:', { ...estimate, isDraft });
      setTimeout(() => {
  ...
  const renderItemRow = (item: EstimateItem, index: number) => (
    <View
      key={item.id}
      style={[
    ...
    >
      ...
      <View style={styles.displayRow}>
        <View className="displayField">
          <Text style={[styles.displayLabel, { color: theme.colors.textSecondary }]}> Item Total </Text>
        </View>
        <View style={[styles.totalContainer, { backgroundColor: `${theme.colors.primary}${themeType === 'dark' ? '15' : '10'}` }]}> <IndianRupee size={14} color={theme.colors.primary} /> <Text style={[styles.totalValue, { color: theme.colors.primary }]}> {item.total.toFixed(2)} </Text> </View>
      </View>
    </View>
  );
        ...
        >
          ...
          >
            <View style={styles.saveButtonContent}>
              ...
              <Text style={styles.saveButtonText}> {isCreatingEstimate ? 'Submitting...' : 'Submit'} </Text>
            </View>
          </LinearGradient>
        </TouchableOpacity>
      </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}> 
      ...
      >
        <SafeAreaView>
          <View style={styles.header}>
            <TouchableOpacity style={styles.backButton} onPress={() => router.back()}> <ArrowLeft size={20} color="rgba(255, 255, 255, 0.9)" /> </TouchableOpacity>
            <View style={styles.headerTitleContainer}> <Text style={styles.headerTitle}>Create Estimate</Text> </View>
        ...
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          <Animated.View entering={FadeInUp.delay(100)}>
            <BlurView intensity={themeType === 'dark' ? 15 : 80} tint={themeType} style={styles.section}>
              ...
              {renderFormInput(
                'Customer',
                customers.find((c) => c.id === estimate.customerId)?.name || '',
                () => {},
              ...
              {renderFormInput(
                ...
                (text) => setEstimate({ ...estimate, estimateNumber: text }),
                'Enter estimate number'
              )}

              <View style={styles.formRow}>
                <View style={[styles.formGroup, { flex: 1, marginRight: 12 }]}> <Text style={[styles.label, { color: theme.colors.textSecondary }]}>Estimate Date</Text>
                  <TouchableOpacity
                    style={[
                      styles.inputContainer,
                      {
                    ...
                    onPress={() => setShowDatePicker('estimateDate')}
                  >
                    <Text style={[styles.textInput, { color: theme.colors.text }]}> {estimate.estimateDate.toLocaleDateString()} </Text>
                    <View style={styles.inputIcon}> <Calendar size={18} color={theme.colors.textSecondary} /> </View>
                  </TouchableOpacity>
                </View>
                <View style={[styles.formGroup, { flex: 1 }]}> <Text style={[styles.label, { color: theme.colors.textSecondary }]}>Valid Till</Text>
                  <TouchableOpacity
                    style={[
                      styles.inputContainer,
                      {
                    ...
                    onPress={() => setShowDatePicker('validTill')}
                  >
                    <Text style={[styles.textInput, { color: theme.colors.text }]}> {estimate.validTill.toLocaleDateString()} </Text>
                    <View style={styles.inputIcon}> <Calendar size={18} color={theme.colors.textSecondary} /> </View>
                  </TouchableOpacity>
                </View>
              </View>
          ...
          <Animated.View entering={FadeInUp.delay(200)}>
            <BlurView intensity={themeType === 'dark' ? 15 : 80} tint={themeType} style={styles.section}>
              <View style={styles.sectionHeader}>
                ...
                <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Estimate Items</Text>
                <TouchableOpacity style={[styles.addButton, { backgroundColor: `#EC489920`, borderColor: `#EC489940` }]} onPress={addItem}> <Plus size={16} color="#EC4899" /> </TouchableOpacity>
              </View>
              {estimate.items.map((item, index) => renderItemRow(item, index))}
            </BlurView>
          </Animated.View>
          ...
          <Animated.View entering={FadeInUp.delay(300)}>
            <BlurView intensity={themeType === 'dark' ? 15 : 80} tint={themeType} style={styles.section}>
              ...
              <View style={styles.formGroup}>
                ...
                }]}> <TextInput style={[styles.textInput, styles.notesInput, { color: theme.colors.text }]} value={estimate.notes} onChangeText={(text) => setEstimate({ ...estimate, notes: text })} placeholder="Add any notes for this estimate..." placeholderTextColor={theme.colors.textSecondary} multiline numberOfLines={3} textAlignVertical="top" /> </View>
              </View>
            </BlurView>
          </Animated.View>
          ...
          <Animated.View entering={FadeInUp.delay(500)}>
            <BlurView intensity={themeType === 'dark' ? 15 : 80} tint={themeType} style={styles.section}>
              <View style={styles.sectionHeader}>
                <IndianRupee size={18} color="#EC4899" />
              ...
              <View style={styles.summaryContainer}>
                <View style={styles.summaryRow}>
                  ...
                  <Text style={[styles.summaryValue, { color: theme.colors.text }]}>₹{estimate.subtotal.toFixed(2)}</Text>
                </View>
                <View style={styles.summaryRow}>
                  <Text style={[styles.summaryLabel, { color: theme.colors.textSecondary }]}>Total Tax</Text>
                  <Text style={[styles.summaryValue, { color: theme.colors.secondary }]}>₹{estimate.totalTax.toFixed(2)}</Text>
                </View>
                <View style={styles.summaryRow}>
                  <Text style={[styles.summaryLabel, { color: theme.colors.textSecondary }]}>Total Discount</Text>
                  <Text style={[styles.summaryValue, { color: '#EF4444' }]}>-₹{estimate.totalDiscount.toFixed(2)}</Text>
                </View>
                <View style={[styles.summaryDivider, { backgroundColor: theme.colors.border }]} />
                <View style={styles.summaryRow}>
                  <Text style={[styles.summaryTotalLabel, { color: theme.colors.text }]}>Total Amount</Text>
                  <Text style={[styles.summaryTotalValue, { color: '#EC4899' }]}>₹{estimate.totalAmount.toFixed(2)}</Text>
                </View>
              </View>
            </BlurView>
          </Animated.View>
      ...
      {renderDropdownModal(
        ...
        customers,
        estimate.customerId,
        (value, label) => setEstimate({ ...estimate, customerId: value, customerName: label || '' })
      )}
      {showDatePicker && (
        <DateTimePicker
          value={showDatePicker === 'estimateDate' ? estimate.estimateDate : estimate.validTill}
          mode="date"
          display="default"
          onChange={(event, selectedDate) => {
            ...
            if (selectedDate) {
              setEstimate({
                ...estimate,
                [showDatePicker === 'estimateDate' ? 'estimateDate' : 'validTill']: selectedDate,
              });
            }
          }}
        />
